...
def load_model():
    ...
    try:
        ...
        if not os.path.exists('models/churn_model_pipeline.pkl'):
            st.error("Model file not found! Please train the model first.")
            st.info("Run: python train.py")
            st.stop()
        
        # Load model pipeline
        model = pd.read_pickle('models/churn_model_pipeline.pkl')
        
        # Load features
        features = pd.read_pickle('models/best_features.pkl')
        
        # Load metadata (if available)
        metadata = None
        if os.path.exists('models/model_metadata.pkl'):
            metadata = pd.read_pickle('models/model_metadata.pkl')
...
# Load Model
model_pipeline, best_features, metadata = load_model()
...
if metadata:
    st.caption(
        ...
        f"Features: {metadata.get('n_features', len(best_features))} | "
        f"Train Date: {metadata.get('train_date', 'N/A')}"
    )
else:
    st.caption(f"Features: {len(best_features)}")
...
with st.sidebar:
    ...
    if metadata:
        ...
        with col1:
            st.metric("Train AUC", f"{metadata.get('auc_train', 0):.3f}")
            st.metric("Test AUC", f"{metadata.get('auc_test', 0):.3f}")
        
        with col2:
            st.metric("Train Acc", f"{metadata.get('accuracy_train', 0):.3f}")
            st.metric("Test Acc", f"{metadata.get('accuracy_test', 0):.3f}")
        
        if 'auc_oot' in metadata:
            st.metric("OOT AUC", f"{metadata.get('auc_oot', 0):.3f}")
    
    st.metric("Total Features", len(best_features))
    
    with st.expander("Features List"):
        st.write(best_features)
    
    st.markdown("---")
    st.markdown("### Links")
    st.markdown("[LinkedIn](https://www.linkedin.com/in/arthur-picolo-dos-reis-00b601210/)")
...
st.markdown(
    ...
    <div style='text-align: center; color: #666;'>
        <p>Churn Prediction System | Powered by Streamlit & Scikit-learn</p>
    </div>
    """,
